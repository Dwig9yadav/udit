...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/Hindsight-Memory-FF6B6B?style=for-the-badge&logo=databricks&logoColor=white" alt="Hindsight" />
  <img src="https://img.shields.io/github/license/udithashhacks/udithashhacks-main?style=for-the-badge" alt="License" />
</p>
...
- **🧠 AI Memory** — Hindsight-powered memory bank remembers past interactions for personalized responses
...
- **🧠 Memory Insights** — view and query the Hindsight memory bank for any user; run AI reflections
...
2. **Import in Vercel**
   ...
   - Import `uditbhatt55/udithashhacks`
...
```bash
# Clone
git clone https://github.com/udithashhacks/udithashhacks-main.git
cd udithashhacks-main
...
│   │   ├── hindsight_service.py# Hindsight memory integration (retain/recall/reflect)
...
│       ├── docurag.py          # RAG search, PDF upload/index/delete
│       ├── memory.py           # Hindsight retain/recall/reflect endpoints
│       ├── feedback.py         # Teacher ↔ Admin feedback
│       ├── chat.py             # Chatroom messages
...
| Method | Endpoint | Description |
|--------|----------|-------------|
| GET | `/api/memory/status` | Check Hindsight connection & bank ID |
| POST | `/api/memory/retain` | Store a memory fact in the user's bank |
| POST | `/api/memory/recall` | Retrieve memories matching a query |
| POST | `/api/memory/reflect` | AI-powered reflection over memory bank |

### Chat
| Method | Endpoint | Description |
|--------|----------|-------------|
| POST | `/api/chat/messages` | Send a chat message (students only) |
| GET | `/api/chat/messages` | Retrieve recent chat messages |
...
- [x] Hindsight memory integration — retain/recall/reflect per user bank (Verifed with 10-sentence load test)
- [x] PII redaction before memory storage (emails & phone numbers)
- [x] Student chatroom with auto-expiry (WebSocket broadcast)
- [x] Rate limiting (60 req/min per IP via SlowAPI)
- [x] Security headers middleware (CSP, HSTS, X-Frame-Options)
- [x] Vercel Ready — Consolidated root requirements and Python path injection for serverless functions

---

## 🗺️ Future Roadmap

- [x] 🔑 **Phone OTP Password Reset** — OTP-based password reset via institution phone
- [ ] 🔐 Google OAuth integration
- [ ] 📱 Mobile-responsive optimization
- [x] 🌐 Real-time collaboration — WebSocket chatroom
- [ ] 📊 Advanced analytics with charts & export
- [ ] 🎥 Video content support
- [ ] 📦 Batch document processing
- [ ] 🌍 Improved Hindi/Hinglish language support
- [ ] 🧠 Hindsight memory-augmented RAG responses (inject recalled context into prompts)
...
# Hindsight Memory (optional — set HINDSIGHT_ENABLED=false to disable)
HINDSIGHT_ENABLED=true
HINDSIGHT_API_KEY=your-hindsight-api-key
HINDSIGHT_BASE_URL=https://api.hindsight.vectorize.io
HINDSIGHT_TIMEOUT=6
HINDSIGHT_DEFAULT_BANK=default
...
| Name | Role |
|------|------|
| **Udit** | Project Lead & System Architect |
| **Raghav** | Frontend & UX Engineer |
| **Shrikant** | Backend & AI Engineer |
| **Dwig** | QA & Memory Engineer |

See [documentation/role.md](documentation/role.md) for full responsibilities.

📦 Repository: https://github.com/udithashhacks/udithashhacks-main
