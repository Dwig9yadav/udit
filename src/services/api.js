...
const getApiBaseUrl = () => {
    ...
    // On Vercel, the API is usually mapped to /api via rewrites
...
const apiFetch = async (endpoint, options = {}) => {
    ...
    // Auto-set Content-Type to JSON if not specified and body is not FormData
...
export const ragAPI = {
    ...
    generateTest: async (pdfId, difficulty = 'medium') => {
        return apiFetch('/rag/generate-test', {
            method: 'POST',
            body: JSON.stringify({ pdf_id: pdfId, difficulty }),
        });
    },

    voiceSearch: async (audioBlob) => {
        const formData = new FormData();
        // Determine extension from MIME type to prevent Groq decoding errors
        let ext = 'webm';
        ...
        return apiFetch('/rag/voice-search', {
            method: 'POST',
            body: formData,
        });
    },
    ...
    getPdfLogbook: async () => {
        return apiFetch('/rag/pdf-logbook', {
            method: 'GET',
        });
    },
};
...
export const memoryAPI = {
    ...
    retain: async (content, factType = 'world_fact', metadata = {}, bankId = null) => {
        return apiFetch('/memory/retain', {
            method: 'POST',
            body: JSON.stringify({
                content,
                fact_type: factType,
    ...
    recall: async (query, limit = 5, bankId = null) => {
    ...
    reflect: async (query, mission = null, directives = [], disposition = {}, bankId = null) => {
        return apiFetch('/memory/reflect', {
            method: 'POST',
            body: JSON.stringify({
                query,
                mission,
