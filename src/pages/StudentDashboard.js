...
import PdfLogBook from './PdfLogBook';
...
const StudentDashboard = () => {
  ...
  const [memoryContextUsed, setMemoryContextUsed] = useState(false);
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      setMemoryContextUsed(results.memory_context_used || false);
      setMemoryFragments(results.memory_context_fragments || []);
  ...
  const startRecording = async () => {
      try {
          ...
          // Let the browser choose its optimal codec instead of hardcoding 'audio/webm' to prevent NotSupportedError crashes
          ...
          setIsRecording(true);
          setOperationStatus('🎙️ Listening...');
          if (synth.speaking) synth.cancel();
      } catch (error) {
          setOperationStatus('Microphone access denied. Please check browser permissions.');
      }
  };
  ...
  const stopRecording = () => {
      if (mediaRecorderRef.current && isRecording) {
          ...
          // DO NOT stop tracks here. Let the onstop event fire first.
          setIsRecording(false);
          setOperationStatus('Processing voice audio with Groq...');
      }
  };

  const handleAudioStop = async () => {
      // Safely stop the hardware microphone tracks now that recording is fully finalized
      if (mediaRecorderRef.current && mediaRecorderRef.current.stream) {
          mediaRecorderRef.current.stream.getTracks().forEach(t => t.stop());
      }

      // Determine the correct mimeType that the browser actually used
      const mimeType = mediaRecorderRef.current?.mimeType || 'audio/webm';
      const audioBlob = new Blob(audioChunksRef.current, { type: mimeType });
      setSearching(true);
      try {
          const response = await ragAPI.voiceSearch(audioBlob);
          setSearchQuery(response.transcript);
          setSearchResults(response.results || []);
          setGeneratedAnswer(response.generated_answer || '');
          setMemoryContextUsed(response.memory_context_used || false);
          setMemoryFragments(response.memory_context_fragments || []);
          setOperationStatus(`Voice search complete. Found ${response.results?.length || 0} results.`);
          const history = await ragAPI.getSearchHistory(50);
          setSearchHistory(history || []);
          
          if (response.generated_answer) {
               const utterance = new SpeechSynthesisUtterance(response.generated_answer);
          ...
          handleError(error, 'Voice search failed.');
          setOperationStatus('Voice search error. Check API key in .env');
      } finally {
          setSearching(false);
      }
  };
    ...
    <div className="dashboard">
      ...
      <aside className="sidebar">
        ...
        <nav className="sidebar-nav">
          ...
          <button
            className={`nav-item ${activeTab === 'pdf-logbook' ? 'active' : ''}`}
            onClick={() => setActiveTab('pdf-logbook')}
          >
            <span className="nav-icon">🗂️</span>
            <span>PDF Log Book</span>
          </button>
          ...
          <button
            className={`nav-item ${activeTab === 'exam-mode' ? 'active' : ''}`}
            onClick={() => setActiveTab('exam-mode')}
            id="nav-exam-mode"
          >
            <span className="nav-icon">📝</span>
            <span>AI Exam Mode</span>
          </button>

          <button
            className={`nav-item ${activeTab === 'memory-insights' ? 'active' : ''}`}
            onClick={() => setActiveTab('memory-insights')}
          >
            <span className="nav-icon">🧠</span>
      ...
      <main className="main-content">
        ...
        <div className="content-area">
                ...
                {generatedAnswer ? (
                  <>
                    <div className="ai-answer-card">
                      <div className="ai-answer-header" style={{ display: 'flex', justifyContent: 'space-between', width: '100%' }}>
                        <div>
                          <span className="ai-icon">✨</span>
                          <h3>AI Generated Answer</h3>
                          {memoryContextUsed && (
                            <span 
                              className="hindsight-badge" 
                              title="Personalized via Hindsight Memory"
                        ...
                        <button 
                          ...
                          className="play-btn"
                          style={{ background: 'rgba(255,107,107,0.2)', border: '1px solid #FF6B6B', padding: '5px 12px', borderRadius: '15px', color: '#fff', cursor: 'pointer' }}
                        >
                          🔊 Play/Stop Audio
                        </button>
                      </div>
                      ...
                      {memoryContextUsed && showMemoryInsights && (
                        <div className="memory-insights-box">
                          <div className="memory-insights-header">
                            <h4>🧠 Hindsight Memory Fragments</h4>
                            <button onClick={() => setShowMemoryInsights(false)}>×</button>
                          </div>
                          <ul className="memory-clues-list">
                            {memoryFragments.map((fragment, idx) => (
                              <li key={idx}>"{fragment}"</li>
                            ))}
                          </ul>
                          <p className="memory-disclaimer">These memories were recalled to tailor this specific answer to your context.</p>
                        </div>
                      )}
                    </div>
          ...
          {/* AI Exam Mode */}
          {activeTab === 'exam-mode' && (
            <div className="section-container exam-mode-section fade-in">
              <TestGenerator />
            </div>
          )}

          {/* PDF Log Book */}
          {activeTab === 'pdf-logbook' && (
          ...
          {activeTab === 'memory-insights' && (
            <section className="tab-content memory-insights-section">
              <MemoryInsights />
            </section>
          )}
